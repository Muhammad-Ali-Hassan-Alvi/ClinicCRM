...
export const createSettingsActions = ({ performDbAction }) => {
  const updateSettings = (settingsData) => {
    const actions = Object.entries(settingsData).map(([key, value]) =>
      supabase
        .from("app_settings")
        .upsert({ key, value: keysToSnakeCase(value) }, { onConflict: "key" })
    );
  ...
  const updateUser = (userId, profileData) =>
    performDbAction(
      () =>
        supabase
          .from("profiles")
          .update(keysToSnakeCase(profileData))
          .eq("id", userId),
