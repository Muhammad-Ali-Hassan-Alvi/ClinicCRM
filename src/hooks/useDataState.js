...
const useDataState = () => {
  ...
  const [instagramConversations, setInstagramConversations] = useState(
    initialState.instagramConversations
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const dbSettings = (appSettingsData || []).reduce((acc, setting) => {
        try {
          acc[setting.key] = Array.isArray(setting.value)
            ? setting.value
            : keysToCamel(setting.value);
        } catch (e) {
      ...
      const combinedSettings = {
        ...
        leadStatuses:
          dbSettings.leadStatuses || initialState.settings.leadStatuses,
        departments:
          dbSettings.departments || initialState.settings.departments,
        ticketStatuses: dbSettings.ticketStatuses || [
          "Open",
          "In Progress",
          "Resolved",
          "Closed",
        ],
        ticketPriorities: dbSettings.ticketPriorities || [
          "Low",
          "Medium",
          "High",
        ],
        agents: dbSettings.agents || [
          "Dr. Smith",
          "Nurse Betty",
          "Admin Staff",
          "Unassigned",
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public" }, (payload) => {
        if (payload.table === "messages") {
          return; // If it's a chat message, do nothing and exit early.
        }
